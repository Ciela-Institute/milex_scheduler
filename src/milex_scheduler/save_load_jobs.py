...
__all__ = ["save_job", "save_bundle", "load_bundle", "transfer_slurm_to_remote", "nearest_bundle_filename"]
...
def save_bundle(
    ...
    for _, job in bundle.items():
        if not isinstance(job, dict):
            raise TypeError("Each job in the bundle must be a dictionary. If you want to save a single job, use save_job() instead.")
    ...
    if append:
        ...
        file_path = os.path.join(user_config['local']['path'], 'jobs', f'{name}_{date}.json')
...
def save_job(
    ...
    if append:
        ...
        except FileNotFoundError: # Create a new empty bundle file and save the job inside it 
            date = datetime.now().strftime(DATE_FORMAT)
            print(f"Did not find a file with the pattern '{bundle_name}_*' in the directory {job_dir}. "
...
def transfer_slurm_to_remote(slurm_name, machine_name: Optional[str] = None, machine_config: Optional[dict] = None) -> None:
    """
...
def load_bundle(name: str, desired_date: Optional[datetime] = None) -> tuple[list, dict, datetime]:
    """
...
def nearest_bundle_filename(name: str, desired_date: Optional[datetime] = None) -> tuple[str, datetime]:
    """
    ...
    files = [f[:-5] for f in os.listdir(jobs_dir) if f.startswith(name) and f.endswith('.json')]
    if not files:
        raise FileNotFoundError(f"No files found with name '{name}' in directory {jobs_dir}")
    dates = []
    for file in files:
        try:
            dates.append(datetime.strptime(file.split('_')[-1], DATE_FORMAT))
        except:
            print(f"Could not parse date from file {file}. Name of file expected to have the format 'name_{DATE_FORMAT}.json' this file will not be considered.")
