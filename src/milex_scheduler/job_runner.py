...
def run_jobs(name: str, machine_config: Optional[dict] = None, date: Optional[datetime] = None):
    """
    ...
    for job in jobs:
        if job.get("script", None) is None:
            ...
            raise ValueError("'name' entry is missing from one of the jobs in the configuration file {job_name}")
    ...
    for job in jobs:
        ...
        if machine != 'local':
            ...
            print(f"Submitted job {job['name']} with ID {job_id} on machine {machine_config['hostname']}")
        else:
            job_id = run_script_locally(slurm_name)
